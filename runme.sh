...
python3 pytorch/auto_thresholds.py calculate_thresholds --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --holdout_fold=$FOLD --model_type=$MODEL_TYPE --loss_type=clip_bce --augmentation='mixup' --learning_rate=1e-3 --batch_size=32 --few_shots=-1 --random_seed=1000 --iteration=60000

# Calculate metrics using the optimal thresholds
python3 pytorch/auto_thresholds.py calculate_metrics --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --holdout_fold=$FOLD --model_type=$MODEL_TYPE --loss_type=clip_bce --augmentation='mixup' --learning_rate=1e-3 --batch_size=32 --few_shots=-1 --random_seed=1000 --iteration=60000
